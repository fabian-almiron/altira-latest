...
export async function getTemplateFilesForExport(
  ...
  if (v0Files && v0Files.length > 0) {
    console.log('🔍 Analyzing v0 files for shadcn components...')
    const shadcnComponents = await getDynamicShadcnComponents(v0Files)
    
    console.log(`📦 Shadcn components received: ${Object.keys(shadcnComponents).length}`)
    console.log(`📦 Component paths:`, Object.keys(shadcnComponents))
    
    for (const [path, content] of Object.entries(shadcnComponents)) {
      console.log(`  → Checking ${path}, shouldSkip: ${shouldSkipFile(path, v0FilePaths)}`)
      if (!shouldSkipFile(path, v0FilePaths)) {
        templatesToAdd[path] = content
        console.log(`    ✅ Added ${path} (${content.length} chars)`)
      } else {
        console.log(`    ❌ Skipped ${path} (v0 already has it)`)
      }
    }
  }

  console.log(`📂 Final templates to add: ${Object.keys(templatesToAdd).length}`)
  console.log(`📂 UI files in templates:`, Object.keys(templatesToAdd).filter(f => f.startsWith('components/ui/')))
